...
#define RC_SYNC_PULSE_US  2500   // gap > this indicates frame sync
...
typedef struct {
    uint32_t edgeCount;
    uint32_t syncCount;
    uint32_t goodPulseCount;
    uint32_t rejectCount;
    uint16_t lastIntervalUs;
    uint16_t lastPwmHighUs;
    uint32_t lastEdgeMs;
    uint8_t sampledHighEdge;
    RC_SignalType signalType;
} RC_Diagnostics;
...
// Snapshot low-level RC decoder diagnostics.
